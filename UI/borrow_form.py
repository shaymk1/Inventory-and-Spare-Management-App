...
class BorrowForm:
    ...
    def _create_form(self):
        ...
        ctk.CTkLabel(title_frame, text="⬇️ Submit", font=("Arial", 22, "bold")).pack()
    ...
    def _update_quantity_info(self, event=None):
        ...
        selected_text = self.spare_combo.get()
        if selected_text and selected_text != "Select a spare...":
            options = self.spare_combo.cget("values")
            try:
                ...
                if selected_index >= 0 and selected_index < len(self.current_spares):
                    ...
                    self.quantity_info_label.configure(
                        text=f"Available quantity: {spare['quantity']}",
                        text_color="green",
                    )
            except ValueError:
                pass  # Selection not found in options
    ...
    def _process_borrow(self):
        ...
        print("DEBUG: _process_borrow() called")

        # Get form values
        if not self.spare_combo:
            MessageDialog.show_error(
                self.main_frame, "Error", "No spares available to borrow"
            )
            return

        # Get selected text from combobox
        selected_text = self.spare_combo.get()
        if not selected_text or selected_text == "Select a spare...":
            MessageDialog.show_error(self.main_frame, "Error", "Please select a spare")
            return

        # Find the index in the combobox options
        options = self.spare_combo.cget("values")
        try:
            selected_index = options.index(selected_text)
        except ValueError:
            MessageDialog.show_error(self.main_frame, "Error", "Invalid selection")
            return
        ...
        if selected_index >= 0 and selected_index < len(self.current_spares):
            ...
            MessageDialog.show_error(
                self.main_frame, "Error", "Selected spare not found in database"
            )
            return
        ...
        if not qty_text:
            MessageDialog.show_error(
                self.main_frame, "Error", "Please enter quantity to borrow"
            )
            return
        ...
        try:
            ...
            if borrow_qty <= 0:
                MessageDialog.show_error(
                    self.main_frame, "Error", "Quantity must be greater than 0"
                )
                return
            ...
            MessageDialog.show_error(
                self.main_frame, "Error", "Quantity must be a number"
            )
            return
        ...
        if not borrower:
            MessageDialog.show_error(
                self.main_frame, "Error", "Please enter borrower name"
            )
            return
        ...
        print(f"DEBUG: Showing confirmation dialog for {borrow_qty} of {spare_name}")
        MessageDialog.show_confirm(
            self.main_frame,
            "Confirm Borrow",
            f"Borrow {borrow_qty} of {spare_name}?\n\nBorrower: {borrower}\nNotes: {notes[:50]}...",
            lambda: self._execute_borrow(
                spare_id, spare_name, borrow_qty, borrower, notes
            ),
        )
        ...
        try:
            ...
            if not history:
                MessageDialog.show_info(
                    self.main_frame, "History", "No borrow history found"
                )
                return
            ...
            dialog.focus_force()  # Give focus
            dialog.attributes("-topmost", True)  # Stay on top temporarily
            ...
            # Title
            ctk.CTkLabel(
                dialog, text="📜 Borrow History", font=("Arial", 18, "bold")
            ).pack(pady=(15, 10))
